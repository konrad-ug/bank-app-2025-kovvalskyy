...
Scenario: User is able to update name of already created account
    Given Account registry is empty
    And I create an account using name: "adam", last name: "nowak", pesel: "88010199999"
    When I update "name" of account with pesel: "88010199999" to "piotr"
    Then Account with pesel "88010199999" has "name" equal to "piotr"

Scenario: Created account has all fields correctly set
    Given Account registry is empty
    When I create an account using name: "jan", last name: "kowalski", pesel: "90010112345"
    Then Account with pesel "90010112345" has "name" equal to "jan"
    And Account with pesel "90010112345" has "surname" equal to "kowalski"
